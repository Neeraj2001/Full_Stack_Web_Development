...
var gpattern = []; // empty array to which we will push the new generated colours

var userclick = []; // will store the userchose every time
...
$(".btn").click(function () { // will detect when any of the buttons are clicked and trigger a handler function
    ...
    var userchose = $(this).attr("id"); // userchose will store the id of the button that got clicked (this)

    userclick.push(userchose);

    playSound(userchose);

    animatePress(userchose); // will add .pressed class to the chosen colour

    checkAnswer(userclick.length - 1); // call after a user has clicked and chosen their answer, passing in the index of the last answer in the user's sequence.

    console.log("userclick: " + userclick.length);
...
function checkAnswer(currentLevel) {
    ...
    if (gpattern[currentLevel] === userclick[currentLevel]) { // check if the most recent user answer is the same as the game pattern. If so then log "success", otherwise log "wrong".

        console.log("success");
        //     var soundRight = new Audio("sounds/" + userchose + ".mp3");
        // soundRight.play();

        if (userclick.length === gpattern.length) { // if the user got the most recent answer right in step 3, then check that they have finished their sequence with another if statement.
...
function nextSequence() { // will generate the next colour
    ...
    userclick = []; // once nextSequence() is triggered, reset the userclick to an empty array ready for the next level.
    ...
    gpattern.push(randomChosenColour); // push selected colour into the empty array
...
function startOver() { // reset the values of level, gpattern and started variables. Will be called at the "wrong" if statement.
